...
document.title = "Verify Admin - Hireflash";

if (
    !window.sessionStorage.getItem("verifyId") ||
...
async function toCreateAdmin() {
    verificationCode.value = `${num1.value}${num2.value}${num3.value}${num4.value}${num5.value}${num6.value}`;

    const response = await fetch(baseEndpoint + "/auth/get-started/verify", {
        method: "POST",
        mode: "cors",
    ...
    if (response.status !== 200) {
        ...
        router.push("/get-started/create-admin");
    }
}
...
<template>
    ...
    <main class="bg-indigo-50 w-full min-h-screen grid place-items-center">
        <form
            ...
            v-on:submit.prevent="toCreateAdmin()"
        >
            <!-- Logo -->
