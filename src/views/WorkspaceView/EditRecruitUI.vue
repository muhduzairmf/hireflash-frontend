...
document.title = "Edit Recruitment - Hireflash";
...
onMounted(async () => {
    ...
    designation.value = jobRecruit.value.designation;
    department.value = jobRecruit.value.department;
    minsalary.value = jobRecruit.value.min_monthly_salary;
    maxsalary.value = jobRecruit.value.max_monthly_salary;
    nationality.value = jobRecruit.value.candidate_nationality;
    eduLevel.value = jobRecruit.value.candidate_min_edu_level;
    experience.value = jobRecruit.value.candidate_min_of_exp;
    lang.value = jobRecruit.value.candidate_lang_req;
    otherReq.value = jobRecruit.value.candidate_other_req;
    jobRespons.value = jobRecruit.value.job_responsibilities;
    otherInfo.value = jobRecruit.value.other_info;
    jobType.value = jobRecruit.value.job_type;
    jobField.value = jobRecruit.value.job_field;
    status.value = jobRecruit.value.recruitment_status;

    arrJobType.value = jobType.value.split(",");
...
// A function to submit updated recruitment
async function editRecruit() {
    ...
    if (response.status !== 200) {
        ...
        successRecruit.value = "edit";

        router.push("/workspace/recruitment/" + jobRecruit.value.id);
    }
}
...
<template>
    ...
    <div class="max-w-7xl mx-auto px-6 max-md:px-3">
        ...
        >
            <TitleBorder title="Edit New Recruitment" />
            <form v-on:submit.prevent="editRecruit()">
                ...
                <div class="mt-6 flex gap-2 max-md:mb-4">
                    <router-link
                        v-bind:to="`/workspace/recruitment/` + jobRecruit.id"
                        class="rounded-md bg-gray-50 border-2 border-indigo-800 text-indigo-800 px-4 py-2 transition ease-in-out focus:scale-95 hover:-translate-y-1 grid place-items-center"
                        type="button"
                    >
                        <p>Cancel</p>
                    </router-link>
